window.onload = function() {
	...
	if (ctx) {
		...
		patImg.onload = function(){
			...
			ctx.fillRect(0, 0, ctx.canvas.width, ctx.canvas.height);//sets width n height so it can be changed via html later and not needed to change in two places
		...
		var strokePat = ctx.createPattern(patCanvas, "repeat"); //larger canvas context
